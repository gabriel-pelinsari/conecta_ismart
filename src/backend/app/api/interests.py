...
@router.get("/my-interests", response_model=UserInterestsOut)
...
@router.post("/my-interests", status_code=status.HTTP_201_CREATED)
def add_interest_to_me(
    payload: InterestCreate,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Adiciona um interesse ao perfil do usuário pelo nome"""
    # Busca ou cria o interesse
    interest = db.query(Interest).filter(Interest.name == payload.name).first()
    if not interest:
    ...
    # Adiciona
    user_interest = UserInterest(user_id=current_user.id, interest_id=interest.id)
    db.add(user_interest)
    db.commit()

    return {"message": "Interesse adicionado com sucesso", "interest": interest.name}

@router.delete("/my-interests/{interest_id}", status_code=status.HTTP_204_NO_CONTENT)
